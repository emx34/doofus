...
Doofus uses **Adlib music files in .TBS format, all the game's music/soundtrack is included in the tbsaplay.zip package(Standalone Adlib player).**
...
■  **Gb2tga.exe** v1.0 working version tested with one direction GB-Image to TGA(TARGA)PHOTOSHOP. This tool can also convert TGA image file to GB-image BACK (!?)
   ...
   **[UPDATE: INTERNAL ONLY - PRIVATE v2.0 version TESTED and the program works perfectly in both directions, 100% correctly working = GB(gameimage)to TGA & TGA to GB(gameimage) ]**
